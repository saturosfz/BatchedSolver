...
# dsolve\_batch() and zsolve\_batch() for solving systems with double-
...
# fast\_solve() that is parameterized by data type and architecture. 
...
# memory. For GPU architectures >= sm\_20 dsolve\_batch can handle 

# systems up to dimensions 76, and zsolve\_batch can handle systems up 
...
# The three kernels used are gauss\_jordan1 (used for dimensions 2 

# through 9), gauss\_jordan2 (used for dimension 10), and gauss2 (for 
...
# The two Gauss-Jordan kernels differ in that for gauss\_jordan1 the 
...
# whereas in gauss\_jordan2 the number of threads is less than the number
...
# four functions smatinv\_batch(), dmatinv\_batch(), cmatinv\_batch(), and

# zmatinv\_batch(). These invert matrices with elements of type float,
...
# All four functions map to a single templatized function matinv\_gje3()
...
# For GPU architectures >= sm\_20 smatinv\_batch can handle matrices up to a

# dimension of 109, cmatinv\_batch() and dmatinv\_batch() can handle matrices

# up to a dimension of 77, and zmatinv\_batch() can handle matrices up to a
